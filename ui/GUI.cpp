...
#include "../pictureDelegate/pictureDelegate.h"
...
//    this->trenchCoatTableWidget->setColumnCount(5);
    ...
    this->undoButton->setFont(adminFont);
    this->redoButton = new QPushButton{"Redo"};
    this->redoButton->setFont(adminFont);
...
//    mainLayout->addWidget(this->trenchCoatTableWidget);
    ...
    buttonsLayout->addWidget(this->graphicalReprButton, 3, 0, 1, 2);
...
UserMode::UserMode(UserService &userService, AbstractShoppingBasket *asp, QWidget* parent): QDialog{parent}, userService{userService}, asp{asp} {
    this->init_user();
    this->connectSignalsAndSlots();
    this->tableModel = new CustomTableModel{this->userService};
    this->basketTableView = new QTableView{};
    this->basketTableView->setModel(this->tableModel);
    this->basketTableView->setWindowTitle("Trench coats shop");
    this->basketTableView->setItemDelegate(new PictureDelegate{});
}
...
void UserMode::init_user() {
    ...
    this->tableButton->setFont(userFont);
    ...
    mainLayout->addWidget(this->tableButton, 4, 0, 1, 2);
...
void UserMode::connectSignalsAndSlots() {
    ...
    QObject::connect(this->tableButton, &QPushButton::clicked, this, &UserMode::open_tableView);
}
...
void UserMode::add_to_basket() {
    ...
    this->populate_list();
    this->update_tabel_in_real_time();
}
...
void UserMode::open_tableView() {
    this->basketTableView->resize(1000, 1000);
    this->basketTableView->resizeColumnsToContents();
    this->basketTableView->resizeRowsToContents();
    this->basketTableView->show();
}

void UserMode::update_tabel_in_real_time() {
